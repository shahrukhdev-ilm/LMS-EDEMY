...
const CourseDetails = () => {
  ...
  return  courseData ? (
    <>
  <div className='flex md:flex-row flex-col-reverse gap-0 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
      <div className='absolute top-0 left-0 w-full h-section-height -z-1 bg-gradient-to-b from -cyan-100/70'></div>
      <div className='max-w-xl z-10 text-gray-500'>
        <h1 className='text-3xl md:text-4xl font-extrabold text-black-700 mb-4 truncate whitespace-nowrap'>
        ...
        <img key={i} src={i < Math.floor(calculateRating(courseData)) ? assets.star : assets.star_blank} alt="star" className='w-3.5 h-3.5' />
         ...
         <p className='text-gray-500'>({courseData.courseRatings.length} {courseData.courseRatings.length === 1 ? 'Rating' : 'Ratings'})</p>
         <p>{courseData.enrolledStudents.length} {courseData.enrolledStudents.length === 1 ? 'Students' : 'Student'}</p>
      </div>
      <p className='text-sm'>Course by <span className='text-blue-600 underline'>Shahrukh</span> </p>
  ...
  <div className='max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px] -ml-2'>
        {
              playerData ?
        ...
        <div className='p-5'>
          ...
          <div className='flex gap-3 items-center pt-2'>
            <p className='text-gray-800 md:text-4xl text-2xl font-semibold'><Dollar />{(courseData.coursePrice - courseData.discount * courseData.coursePrice / 100).toFixed(2)}</p>
            <p className='md:text-lg text-gray-500 line-through'><Dollar />{courseData.coursePrice}</p>
         ...
         <div className='pt-6'>
          ...
          <ul className='ml-4 pt-2 text-default list-disc text-gray-500'>
